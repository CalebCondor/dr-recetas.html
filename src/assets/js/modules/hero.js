...
export const initTyping = () => {
    ...
    const type = () => {
        ...
        if (!isDeleting && charIdx === currentWord.length) {
            typeSpeed = 2000;
            isDeleting = true;
            if (animate) animate(typingText, { scale: [1, 1.05, 1] }, { duration: 0.3 });
...
const renderCards = () => {
    ...
    const windowStartIndex = Math.max(0, Math.min(activeIndex, consultations.length - WINDOW_SIZE));
    ...
    consultations.forEach((_, index) => {
        ...
        const isVisible = index >= windowStartIndex && index < windowStartIndex + WINDOW_SIZE;
        ...
        if (isActive) {
            inner.className = "card-inner relative overflow-hidden rounded-2xl p-4 lg:p-5 px-5 lg:px-10 bg-white flex flex-col justify-center min-h-[70px] shadow-2xl ring-1 ring-black/5";
            text.className = "card-text text-base lg:text-lg font-bold text-left tracking-tight transition-colors duration-300 text-[#0D4B4D]";
        } else {
            inner.className = "card-inner relative overflow-hidden rounded-2xl p-4 lg:p-5 px-5 lg:px-10 bg-white flex flex-col justify-center min-h-[70px] shadow-sm";
            text.className = "card-text text-base lg:text-lg font-bold text-left tracking-tight transition-colors duration-300 text-slate-600";
        }
    });
};
...
export const initCards = () => {
    ...
    consultations.forEach((item, index) => {
        ...
        card.id = `card-${index}`;
        card.className = "absolute top-0 left-0 w-full cursor-pointer will-change-transform";
        card.style.opacity = "0";

        card.innerHTML = `
            <div class="card-inner relative overflow-hidden rounded-2xl p-4 lg:p-5 px-5 lg:px-10 
                        bg-white flex flex-col justify-center min-h-[70px] transition-all duration-500">
